...
#else
#pragma message ("__CUDA_ARCH__ defined")
#endif
...
__device__ __forceinline__ void XOR_ROTATE_ADD(uint32_t& dst, uint32_t& s1, uint32_t& s2,  int amt) {
    uint32_t tmp = s1+s2; dst ^= __funnelshift_l(tmp, tmp, amt);
}
#endif
...
template <int ALGO, MemoryAccess SCHEME> __global__ void maxwell_scrypt_core_kernelA(const uint32_t *d_idata, int begin, int end) {
...
template <int ALGO, MemoryAccess SCHEME> __global__ void maxwell_scrypt_core_kernelA_LG(const uint32_t *d_idata, int begin, int end, unsigned int LOOKUP_GAP) {
...
template <int ALGO, MemoryAccess SCHEME> __global__ void maxwell_scrypt_core_kernelB(uint32_t *d_odata, int begin, int end) {
...
template <int ALGO, MemoryAccess SCHEME> __global__ void maxwell_scrypt_core_kernelB_LG(uint32_t *d_odata, int begin, int end, unsigned int LOOKUP_GAP) {
...
MaxwellKernel::MaxwellKernel() : KernelInterface()
{
}

void MaxwellKernel::set_scratchbuf_constants(int MAXWARPS, uint32_t** h_V)
{
    checkCudaErrors(cudaMemcpyToSymbol(c_V, h_V, MAXWARPS*sizeof(uint32_t*), 0, cudaMemcpyHostToDevice));
}

bool MaxwellKernel::run_kernel(dim3 grid, dim3 threads, int WARPS_PER_BLOCK, int thr_id, cudaStream_t stream, uint32_t* d_idata, uint32_t* d_odata, unsigned int N, unsigned int LOOKUP_GAP, bool interactive, bool benchmark, int texture_cache)
{
    bool success = true;
    ...
    do 
    {
        if (LOOKUP_GAP == 1) switch(opt_algo) {
            case ALGO_SCRYPT:      maxwell_scrypt_core_kernelA<ALGO_SCRYPT     , ANDERSEN><<< grid, threads, 0, stream >>>(d_idata, pos, min(pos+batch, N)); break;
            case ALGO_SCRYPT_JANE: maxwell_scrypt_core_kernelA<ALGO_SCRYPT_JANE, SIMPLE  ><<< grid, threads, 0, stream >>>(d_idata, pos, min(pos+batch, N)); break;
        } else switch(opt_algo) {
            case ALGO_SCRYPT:      maxwell_scrypt_core_kernelA_LG<ALGO_SCRYPT     , ANDERSEN ><<< grid, threads, 0, stream >>>(d_idata, pos, min(pos+batch, N), LOOKUP_GAP); break;
            case ALGO_SCRYPT_JANE: maxwell_scrypt_core_kernelA_LG<ALGO_SCRYPT_JANE, SIMPLE   ><<< grid, threads, 0, stream >>>(d_idata, pos, min(pos+batch, N), LOOKUP_GAP); break;
        } 
    ...
    {
        if (LOOKUP_GAP == 1) switch(opt_algo) {
            case ALGO_SCRYPT:      maxwell_scrypt_core_kernelB<ALGO_SCRYPT     , ANDERSEN><<< grid, threads, 0, stream >>>(d_odata, pos, min(pos+batch, N)); break;
            case ALGO_SCRYPT_JANE: maxwell_scrypt_core_kernelB<ALGO_SCRYPT_JANE, SIMPLE  ><<< grid, threads, 0, stream >>>(d_odata, pos, min(pos+batch, N)); break; }
        else switch(opt_algo) {
            case ALGO_SCRYPT:      maxwell_scrypt_core_kernelB_LG<ALGO_SCRYPT     , ANDERSEN><<< grid, threads, 0, stream >>>(d_odata, pos, min(pos+batch, N), LOOKUP_GAP); break;
            case ALGO_SCRYPT_JANE: maxwell_scrypt_core_kernelB_LG<ALGO_SCRYPT_JANE, SIMPLE  ><<< grid, threads, 0, stream >>>(d_odata, pos, min(pos+batch, N), LOOKUP_GAP); break; }
